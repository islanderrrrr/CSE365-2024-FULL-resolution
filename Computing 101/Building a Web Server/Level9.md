...
# 思路
这一题便运用到了一个新的概念:**子父进程**  (研究了好久/(ㄒoㄒ)/~~)  
我来好生讲讲  
**子父进程**:  
- 子进程父进程执行顺序:父进程执行时，运用系统调用57号，产生一个子进程，父进程挂起，待子进程执行完毕，父进程继续执行
- 子进程的pid为父进程pid+1
- 父进程执行fork调用时，返回子进程的pid，子进程执行fork调用时，返回0

以上为自用的经验，可能有错误，但是是解此题的关键
